...
class ToolSpec(BaseModel):
    ...
    normalize_output_json_strings: bool = False
...
class ToolCatalog(BaseModel):
    ...
    def _from_prepared(
        ...
        for tool, public_name, server, namespace, method, kind, chain_id in prepared:
            ...
            normalize_output_json_strings = bool(
                kind == "mcp_tool"
                and output_schema
                and _schema_allows_json_string(adapter_schema if wrap_output else wrapped_schema)
            )
            builder = StubBuilder(
                public_name,
                input_schema,
                output_schema,
                unwrap_fastmcp_result=wrap_output,
                normalize_json_string_output=normalize_output_json_strings and wrap_output,
            )
            ...
            spec = ToolSpec(
                ...
                normalize_output_json_strings=normalize_output_json_strings,
            )
    ...
    def normalize_result(self, tool_name: str, result: mcp_types.CallToolResult) -> JsonValue:
        ...
        if spec.output_schema:
            ...
            if adapter is None:
                dumped = JSON_VALUE_ADAPTER.validate_python(to_jsonable_python(structured))
            else:
                validated = adapter.validate_python(structured)
                dumped = JSON_VALUE_ADAPTER.validate_python(
                    adapter.dump_python(validated, mode="json")
                )
            if (spec.output_wrap_result or wrap_from_meta) and spec.normalize_output_json_strings:
                return _normalize_json_value(dumped)
            return dumped
...
def _schema_allows_json_string(schema: JsonSchema | None) -> bool:
    if schema is None or schema is True:
        return True
    if schema is False:
        return False
    for key in ("anyOf", "oneOf"):
        raw_members = schema.get(key)
        if isinstance(raw_members, list) and any(
            isinstance(member, (dict, bool)) and _schema_allows_json_string(member)
            for member in raw_members
        ):
            return True
    raw_type = schema.get("type")
    if raw_type is None:
        return not ("properties" in schema or "additionalProperties" in schema or "items" in schema)
    if raw_type == "string":
        return True
    return isinstance(raw_type, list) and "string" in raw_type
...
class StubBuilder:
    def __init__(
        ...
        output_schema: JsonObject | None,
        *,
        unwrap_fastmcp_result: bool = False,
        normalize_json_string_output: bool = False,
    ) -> None:
        self.tool_name = tool_name
        self.input_schema = input_schema
        self.output_schema = output_schema
        self.unwrap_fastmcp_result = unwrap_fastmcp_result
    ...
    def build(self) -> tuple[str, str, list[str]]:
        ...
        if self.unwrap_fastmcp_result and self.output_schema:
