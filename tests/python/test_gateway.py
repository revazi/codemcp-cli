...
async def test_force_discover_refreshes_only_the_selected_server(
    ...
    try:
        ...
        assert (
            next(
                upstream for upstream in first_status.upstreams if upstream.name == "beta"
            ).discovered
            is True
        )
        ...
        runtime.settings_path.write_text(json.dumps({"disabledTools": {"beta": ["save_number"]}}))
...
async def test_gateway_lazy_connections_cache_facade_and_cleanup(
    ...
    async with client:
        ...
        beta_search = await client.call_tool("search", {"query": "number", "server": "beta"})
        beta_search_data = structured_data(beta_search.structured_content)
        assert [item["name"] for item in beta_search_data["results"]] == ["beta_save_number"]
        ...
        assert "AlphaGetNumberArgs" in next(
            item["stub"] for item in search_data["results"] if item["name"] == "alpha_get_number"
        )
...
async def test_saved_chains_are_typed_composable_and_recursion_is_bounded(
    ...
    try:
        ...
        assert {summary.name: summary.tool_count for summary in matches.servers} == {"chains": 2}
        ...
        assert blocked.failure_stage == "preflight"
        dependent = next(
            view for view in runtime.chains.list().chains if view.chain.name == "double_countdown"
        )
