...
async def test_oauth_uses_native_fastmcp_persistence_and_refresh(
    ...
    flow = first_auth.async_auth_flow(httpx.Request("POST", "https://mcp.linear.app/mcp"))
...
def test_config_allows_every_server_to_be_disabled(tmp_path: Path) -> None:
    ...
    assert [(server.name, server.enabled) for server in normalized.servers] == [("only", False)]
...
def test_recursive_json_schema_refs_stop_at_json_value() -> None:
    ...
    assert (
        "results: list[str | float | bool | None | dict[str, JsonValue] | list[JsonValue]]" in stub
    )
    assert "Any" not in catalog.type_stubs
...
def test_runtime_argument_validation_preserves_omitted_optional_fields() -> None:
    ...
    assert catalog.validate_arguments("linear_list_issues", {"limit": 1}) == {"limit": 1}
...
def test_catalog_validates_arguments_and_unknown_schema_names() -> None:
    ...
    assert catalog.validate_arguments("linear_update_issue", {"id": "x", "priority": 2}) == {
        "id": "x",
        "priority": 2,
    }
...
def test_declared_structured_string_outputs_remain_string_typed() -> None:
    catalog = ToolCatalog.from_server_tools({
        "example": [
            make_tool(
                "read",
    ...
    stub = catalog.tools["example_read"].stub
    assert "value: str" in stub
