...
@pytest.mark.asyncio
async def test_normalize_result_parses_dynamic_text_and_wrapped_json_strings() -> None:
    current = catalog()
    ...
    wrapped_object_catalog = ToolCatalog.from_server_tools({
        "example": [
            tool(
                "read_wrapped_object",
                {},
    ...
    wrapped_object = mcp_types.CallToolResult(
        content=[],
        structuredContent={"result": {"value": "null"}},
        isError=False,
    )
    assert wrapped_object_catalog.normalize_result(
        "example_read_wrapped_object", wrapped_object
    ) == {"value": "null"}
    ...
    assert declared_catalog.normalize_result("example_read", declared) == {"value": "null"}
